...
cache_peer {hostname} parent {port} 0 no-query weighted-round-robin weight=2 originserver forceddomain={hostname} max-conn=1 name={name}
...
with open('mirrors.txt') as f:
    ...
    for line in f:
        ...
        if url.hostname != 'us.archive.ubuntu.com':
            continue

        # # Use only official mirrors
        # if not url.hostname.endswith('.ubuntu.com'):
        #     continue
